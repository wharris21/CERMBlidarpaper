...
terra::writeVector(sample_points, here::here("data/sample_points.shp"), overwrite=TRUE)
...
data$firetype <- substr(data$Label, 8, 30)

#Set na fireyears to 1970 (long unburnt)
data$fireyear[is.na(data$fireyear)] <- 1970
data$firetype[is.na(data$firetype)] <- "Wildfire"

#Keep most recent fire at each point
data <- data %>% 
  group_by(ID) %>%
  dplyr::filter(fireyear == max(fireyear)) %>% 
  ungroup()
